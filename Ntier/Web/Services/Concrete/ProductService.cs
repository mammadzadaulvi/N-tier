...
namespace Web.Services.Concrete
{
    public class ProductService : IProductService
    {
        ...
        public async Task<bool> CreateAsync(ProductCreateVM model)
        {
            ...
            {
                _modelState.AddModelError("MainPhoto", "The image must be img format");
                return false;
            }
            if (!_fileService.CheckSize(model.MainPhoto, 300))
            {
                _modelState.AddModelError("MainPhoto", "The File is bigger than 300kb");
                return false;
            }
            ...
            foreach (var photo in model.Photos)
            {
                ...
                {
                    _modelState.AddModelError("Photos", $"{photo.FileName} The image must be img format");
                    hasError = true;

                }
                else if (!_fileService.CheckSize(photo, 300))
                {
                    _modelState.AddModelError("Photos", $"{photo.FileName} is bigger than 300kb");
                    hasError = true;
                }
            }
        ...
        public async Task<ProductUpdateVM> GetUpdateModelAsync(int id)
        {
            ...
            var model = new ProductUpdateVM
            {
                ...
                MainPhotoPath = product.MainPhotoName,
